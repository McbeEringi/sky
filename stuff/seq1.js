...
a2c=(b)=>{
	...
	core=(y,p=0,l=1,ind=[])=>{
		...
		for(let[yi,z]of y.entries()){
			let tmp=pos;
			if(typeof z=='object'){
				pos+=13;core(z,p,l*z.length,[...ind,yi]);pos+=12;
				...
				if(-15<pos&&pos<w){
					...
					if(z)z.split(',').forEach(n=>{
						...
						note.push([col,pos+1,225-Number(n2i[n])*16]);//240-16+1
					});
				}
			...
			if(w*.5-pad<pos&&tmp<w*.5+pad)calced.sel.push({ind:[...ind,yi],pos:[tmp,pos]});
	...
	pos+=16;
	if(calc){calced.scr=scr.scrollLeft;calced.dat=[];calced.sel=[];}
	core(main.scores);pos+=w*.5;
	note.forEach(y=>frr(ctx,...y,14,14,4));
...
init=()=>{
	...
	calced={};
	scrw.style.width=window.onresize()+'px';
	stopbtn.onclick();
};
...
};
{
	...
	window.onresize=()=>{c.width=c.parentNode.clientWidth*res;c.height=240*res;ctx.scale(res,res);return a2c(1);};
...
main.sc=-2;
main.bpm=120;
init();log.textContent='build: 2105280';
