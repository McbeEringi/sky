...
const urlq=Object.fromEntries(location.search.slice(1).split('&').filter(y=>y).map(x=>x.split('=',2))),
	...
	pwa_a=(x=document)=>x.querySelectorAll('a[href]').forEach(e=>(e.ontouchstart||(e.ontouchstart=_=>_),'pwa'in urlq&&(e.removeAttribute('target'),e.matches('[href*="://"],[href*="?pwa"]')||e.setAttribute('href',e.getAttribute('href')+'?pwa'))));
gload();
