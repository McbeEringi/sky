...
seq = new Tone.Sequence((time,note)=>{
	note=note.split(',');
	curset();scrset();kbset(note);//Tone.Draw.schedule(()=>{},time);
	if(note[0]){
...
urdo=x=>{
	...
	seq.events=main.scores=JSON.parse(urstack[1]);if(rawedit.checked)rawtxt.value=urstack[1];requestIdleCallback(render);kbset();
};
